...
결제 실패가 곧 즉시 차단은 아닙니다. `org_entitlement.status`에 `GRACE`(유예)가 있어서, 결제가 잠깐 실패해도 유예 기간 동안 서비스가 유지됩니다([[explainers/gate-b-billing-grace|gate-b-billing-grace]]). "결제 실패 → 즉시 차단"이라는 거친 fail-closed 대신, "유예 배너 후 차단"이라는 단계적 저하를 둡니다.
