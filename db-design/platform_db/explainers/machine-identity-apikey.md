...
> ⚠️ **구현 상태**: api_key는 **전체 미구현(설계 확정 단계)**입니다([[requirements]] P6 — 전 항목 ⚠️). 스키마 DDL(§J)과 정책은 확정됐지만 코드는 아직 없습니다. 이 문서는 "왜 이렇게 설계했는가"를 공부하는 용도이고, 코드 예시는 *구현 시 이렇게 된다*는 설계 의도입니다.
