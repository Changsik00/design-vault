...
>
> **2026-05-31 — 상태 축 정정**: 이 저장소는 *설계 문서*이므로 점수는 "구현률"이 아니라 **설계 성숙도**다. 스키마 DDL이 있는 consent(§I)·api_key(§J)는 "미구현"이 아니라 **설계 확정(✅)**으로 재분류(과거 ⚠️ 11건 폐기, 실제 *미설계* 갭은 §4 운영의 🔴뿐). 실행 코드는 이 저장소 범위 밖.  
