...
| 약어 | 풀이 | 한 줄 | 정본 |
|---|---|---|---|
| **ABAC** | Attribute-Based Access Control | 속성(소유·한도·테넌트·IP)으로 인가 | [[role-capability]] |
| **ACID** | Atomicity·Consistency·Isolation·Durability | DB 트랜잭션 4대 보장 | [[consistency-model]] |
| **BFLA** | Broken Function Level Authorization | 기능/엔드포인트 단위 인가 누락 (OWASP API#5) | [[bola-object-authz]] |
| **BOLA** | Broken Object Level Authorization | 객체(1건) 단위 인가 누락 (OWASP API#1) | [[bola-object-authz]] |
| **CAP** | CAP theorem | 분산 시 일관성·가용성·분단내성 중 둘 | [[consistency-model]] |
| **CAS** | Compare-And-Set | 조건부 UPDATE(전제를 WHERE에)로 race 방지 | [[concurrency-control]] |
| **CASL** | — | JS 권한 라이브러리(ability·can()) | [[casl-ability]] |
| **CIDR** | — | IP 대역 표기(`203.0.113.0/24`) | [[machine-identity-apikey]] |
| **DDL** | Data Definition Language | CREATE/ALTER/DROP (migrator만 보유) | [[least-privilege-grant]] |
| **DEK / KEK** | Data / Key Encryption Key | 봉투 암호화 2겹 키 | [[secret-encryption]] |
| **GIN** | Generalized Inverted Index | JSONB `@>` 포함 검색 인덱스 | [[json-column]] |
| **HMAC** | Hash-based MAC | 공유비밀 서명 검증(webhook 수신; api_key 인증엔 미사용) | [[machine-identity-apikey]] |
| **IDOR** | Insecure Direct Object Reference | BOLA의 옛 이름·부분집합 ("DB 키 노출 해킹") | [[bola-object-authz]] |
| **ISMS-P** | 정보보호 관리체계 인증(KR) | audit 5년·긴급접근 감사 요구 | [[break-glass]] |
| **ISO 27001** | ISO/IEC 27001:2022 | 정보보안 경영시스템 — Annex A 기술통제 근거 | [[architecture]] |
| **JWT** | JSON Web Token | claims 담은 토큰(Firebase TTL ~1h) | [[perm-version-propagation]] |
| **KMS** | Key Management Service | 마스터키(KEK) 보관·관리 | [[secret-encryption]] |
| **N+1** | — | 부모1+자식N 쿼리 폭발 함정 | [[orm-testing-drizzle]] |
| **PII** | Personally Identifiable Information | 개인식별정보(PIPA 보호 대상) | [[secret-encryption]] |
| **PIPA** | Personal Information Protection Act | 한국 개인정보보호법 | [[pipa-consent]] |
| **RBAC** | Role-Based Access Control | 역할(`role_code`→`ROLE_PERMISSION`)로 인가 | [[role-capability]] |
| **ReBAC** | Relationship-Based Access Control | 관계·위임(`delegation_grant`)으로 인가 | [[rebac-delegation]] |
| **RLS** | Row-Level Security | DB가 행 단위 테넌트 격리를 강제 | [[multitenancy-rls]] |
| **SLI / SLO / SLA** | Service Level Indicator/Objective/Agreement | 측정값 / 내부목표 / 외부계약 | [[observability-slo]] |
| **SOC2** | — | 보안 운영 통제 인증(break-glass 점검) | [[break-glass]] |
| **SPOF** | Single Point of Failure | platform_db = 인가 단일 장애점 | [[observability-slo]] |
| **SSOT** | Single Source of Truth | 단일 진실 원천 | [[architecture]] |
| **TTL** | Time To Live | 캐시·토큰 유효 수명 | [[perm-version-propagation]] |
| **ULID** | Universally Unique Lexicographically Sortable ID | 시간순 정렬 26자 외부 식별자 | [[pk-ulid-strategy]] |
| **WORM** | Write Once Read Many | 한 번 쓰면 수정·삭제 불가(append-only) | [[audit-hash-chain]] |
| **2PC** | Two-Phase Commit | 분산 트랜잭션 프로토콜(단일 DB라 미사용) | [[consistency-model]] |
